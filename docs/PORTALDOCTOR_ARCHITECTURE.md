...
apply error rolls back those completed steps in reverse order. The tenth
bounded slice makes the adapter result explicit: `Applied`,
`DefinitelyNotApplied` and `OutcomeUnknown`. A definitely-not-applied current
step preserves the ninth-slice behavior, while an unknown current step is
treated as potentially applied and is rolled back before earlier completed
steps. Rollback failures remain separately typed and every attempted rollback
